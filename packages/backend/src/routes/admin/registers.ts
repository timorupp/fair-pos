...
export async function registersAdminRoute(app: FastifyInstance): Promise<void> {
  ...
  /** GET /api/admin/registers — list all registers with their printer name. */
  app.get('/', async (_req, reply) => {
    const result = await query(`
      SELECT r.id, r.name, r.type, r.printer_id, r.created_at,
  ...
  app.post('/', async (req, reply) => {
    const body = req.body as { name?: string; type?: string; printer_id?: string | null };
    ...
    const result = await query(
      `INSERT INTO register (name, type, printer_id)
       VALUES ($1, $2, $3)
       RETURNING id, name, type, printer_id, created_at`,
      [body.name, body.type, body.printer_id ?? null],
    );
  ...
  app.put('/:id', async (req, reply) => {
    ...
    const result = await query(
      `UPDATE register
       ...
       WHERE id = $4
       RETURNING id, name, type, printer_id, created_at`,
      [body.name ?? null, body.type ?? null, body.printer_id !== undefined ? body.printer_id : null, id],
    );
  ...
  app.post('/:id/transactions', async (req, reply) => {
    ...
    const result = await query(
      ...
      [id, req.user.id, body.type, body.amount, body.note ?? null],
    );
    return reply.status(201).send(result.rows[0]);
  });
}
