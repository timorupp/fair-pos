/**
 * Vendored copy of the "gdpdu-01-09-2004.dtd" — the DTD that
 * `index.xml`'s `<!DOCTYPE DataSet SYSTEM "...">` declaration references.
 * Sourced verbatim from the official DSFinV-K 2.4 download package
 * (`02_index.xml/gdpdu-01-09-2004.dtd`), published by the Bundeszentralamt
 * für Steuern at bzst.de (Task #122). Must be shipped alongside `index.xml`
 * in the export ZIP — the DTD reference otherwise doesn't resolve on the
 * medium, per the Beschreibungsstandard's own requirement that "Die DTD
 * muss sich in demselben Verzeichnis [...] befinden wie die Datei index.xml."
 */

/** Filename `index.xml`'s DOCTYPE declaration expects to find alongside it. */
export const GDPDU_DTD_FILENAME = 'gdpdu-01-09-2004.dtd';

/** Verbatim DTD content — do not reformat, this is a vendored external document. */
export const GDPDU_DTD_CONTENT = `<?xml version="1.0" encoding="UTF-8"?>
<!--Versions available:
1.1 (August-01-2002)
