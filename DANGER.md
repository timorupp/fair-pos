...
| ID | Schwere | Bereich | Beschreibung | Vorschlag | Gefunden |
|----|---------|---------|--------------|-----------|----------|
| D-009 | hoch | Compliance | QR-Code-Payload (`receipt/qr.ts`) verwendet ein selbst-erfundenes Format (semikolon-getrennt). **Konkretisiert 2026-08-05:** DSFinV-K v2.4 Anhang I schreibt das Format exakt vor (`<qr-code-version>;<kassen-seriennummer>;<processType>;<processData>;<transaktions-nummer>;<signatur-zaehler>;<start-zeit>;<log-time>;<sig-alg>;<log-time-format>;<signatur>;<public-key>`) — inkl. TSE-Zertifikatsfelder (Signaturalgorithmus, Public Key), die `native/tse-cli` aktuell nicht ausliest. Zusammengeführt mit Task #46 (auch `processData`-Format für Kassenbeleg-V1 betroffen). | Bereits als Task #46 erfasst, mit vollständigem Zitat in `docs/Rechtliche-Anforderungen.md` Abschnitt 6.5. Vor dem ersten Echteinsatz mit TSE zwingend umsetzen. | 2026-06-24, konkretisiert 2026-08-05 |
...
| D-016 | niedrig | UX | Bedienungskasse-User landen auf der Bonkasse-Route und sehen einen statischen Hinweis statt der für ihren Kassentyp passenden UI. | Wird mit Task #6 (Bedienungskasse-UI) gelöst. | 2026-06-24 |
...
| ID | Schwere | Bereich | Beschreibung | Vorschlag | Gefunden |
|----|---------|---------|--------------|-----------|----------|
| D-017 | mittel | Compliance | Beim "100 % Rabatt"-Pfad wird im Storno-Endpoint aktuell KEINE 0-€-Invoice + TSE-Eintrag erstellt — nur die order_items werden auf `status='free'` gesetzt. Anforderung verlangt aber Zahlungsbeleg mit 0 € + TSE-Verbuchung. | Bei Task #4 (TSE-Integration) im selben Endpoint die Invoice anlegen, sobald `booking_type='free_of_charge'`. | 2026-06-24 |
